...
public class DecisionReasonReader extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		// Read the file
		from("file://" + inputFile + "?fileName=decisionreason.txt&noop=true").log("reads file").split()
				.tokenize("\n", 1, true).unmarshal(bindyDataFormat).process(new DecisionReasonProcessors())
				.log("toKafkaTopic").to(util.buildKafkaEndpointUri("kafka:redcard-enrollment-archives-decisionreason"));
	}
